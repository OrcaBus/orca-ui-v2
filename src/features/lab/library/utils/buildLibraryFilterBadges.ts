...
export interface BuildLibraryFilterBadgesParams {
...
/**
 * Build active filter badges for AdvancedFilterBar from library search + filter state.
 * Badge type drives PillTag variant: search → neutral, range/filter → blue.
 */
export function buildLibraryActiveFilterBadges({
  filterFields,
  search,
  setSearchQuery,
  filterValues,
  setFilterValues,
}: BuildLibraryFilterBadgesParams): FilterBadge[] {
