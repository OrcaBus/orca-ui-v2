...
import type { LibraryListQueryParams } from '../../shared/api/lab.api';
...
export const LIBRARY_FILTER_KEYS = [
  PARAM_ORCABUS_ID,
  PARAM_LIBRARY_ID,
  PARAM_ASSAY,
  PARAM_INDIVIDUAL_ID,
...
/**
 * Library list page state driven by URL query params.
 * Params: search, filters, and server-side pagination.
 */
export function useLibraryQueryParams() {
  const {
    params,
    setParams,
    pagination,
  ...
  const clearAllFilters = useCallback(() => {
    setParams({
      ...
      ...Object.fromEntries(LIBRARY_FILTER_KEYS.map((k) => [k, undefined])),
    });
  }, [setParams]);
