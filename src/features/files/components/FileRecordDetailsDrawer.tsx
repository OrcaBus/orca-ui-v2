...
export function FileRecordDetailsDrawer({ file, onClose }: FileRecordDetailsDrawerProps) {
  ...
  return (
    <>
      ...
      <div className='fixed top-0 right-0 z-50 flex h-full w-full max-w-xl flex-col bg-white shadow-2xl dark:border-l dark:border-[#2d3540] dark:bg-[#111418]'>
        {/* Header */}
          ...
          <div className='space-y-6'>
            ...
            {(file.storageClass || file.archiveStatus || file.eTag || file.sha256) && (
              <div>
                ...
                {file.archiveStatus && (
                  <InfoRow label='Archive Status'>{file.archiveStatus}</InfoRow>
                )}
                {file.eTag && (
                  <InfoRow label='ETag'>
