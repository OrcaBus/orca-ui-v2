...
export interface FileRecordDetailsDrawerProps {
  /** The file to show. May be `null` while closed; the drawer stays mounted regardless. */
  file: S3Record | null;
  isOpen: boolean;
  onClose: () => void;
}
...
export function FileRecordDetailsDrawer({ file, isOpen, onClose }: FileRecordDetailsDrawerProps) {
  // Keep the last file so the panel keeps its content while it animates closed.
  const shown = useLastPresent(file);

  return (
    <DrawerFrame
      isOpen={isOpen}
      onClose={onClose}
      title={shown ? getFilename(shown.key) : ''}
      subtitle={shown ? <span className='font-mono text-xs'>{shown.s3ObjectId}</span> : undefined}
      size='md'
      closeLabel='Close file details'
    >
      {shown && <FileRecordDetailsBody file={shown} />}
    </DrawerFrame>
  );
}

function FileRecordDetailsBody({ file }: { file: S3Record }) {
  ...
  return (
    <div className='space-y-6'>
      ...
      <div>
        ...
        {file.eventTime && <InfoRow label='Event Time'>{formatTableDate(file.eventTime)}</InfoRow>}
