...
function getUniqueAuthors(maps: MapSummary[]): string[] {
  return [...new Set(maps.map((d) => d.createdBy))].sort();
}
...
function CreateMapCard({ onClick }: { onClick: () => void }) {
  return (
    <button
      type='button'
      onClick={onClick}
    ...
    >
      ...
      <div className='text-sm font-semibold text-slate-700 dark:text-slate-300'>Create New Map</div>
      <div className='mt-1 text-xs text-slate-400 dark:text-[#6b7a8d]'>
        Start building a new map from a template or scratch.
      </div>
    </button>
  );
}
...
export function MapListPage() {
  ...
  const handleCreateMap = useCallback((data: MapFormData) => {
    // TODO: Persist to backend / update local state
    console.log('Create map:', data);
  }, []);

  const authors = useMemo(() => getUniqueAuthors(MAP_LIST), []);
      ...
      <div className='mx-auto max-w-6xl px-6 py-8'>
        ...
        <div className='mb-8 flex items-start justify-between'>
          <div>
            ...
            <p className='mt-1 text-sm text-slate-500 dark:text-[#9dabb9]'>
              Explore and plan system architecture through interactive maps.
            </p>
          </div>
        ...
        <div className='mb-6 flex flex-wrap items-center gap-3'>
          ...
          <FilterSelect
            ...
            onChange={(v) => setStatusFilter(v as MapStatus | 'all')}
          />
        </div>

        {/* Results count */}
        {filtered.length !== MAP_LIST.length && (
          <div className='mb-4 text-xs text-slate-400 dark:text-[#6b7a8d]'>
            Showing {filtered.length} of {MAP_LIST.length} maps
          </div>
        )}
        ...
        {viewMode === 'grid' ? (
          <div className='grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3'>
            {filtered.map((m) => (
              <MapCard key={m.mapId} map={m} />
            ))}
            <CreateMapCard onClick={handleOpenCreateModal} />
          </div>
        ) : (
          <div className='space-y-3'>
            {filtered.map((m) => (
              <MapListRow key={m.mapId} map={m} />
            ))}
          </div>
        )}
