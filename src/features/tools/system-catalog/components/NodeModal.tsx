...
import type { EdgeType, NodeFormData, CatalogNodeData } from '../types/system-catalog.types';
...
export function NodeModal({
  ...
  const availableParents = Object.entries(allNodes).filter(
    ([id]) => id !== editingId && !parentLinks.some((parentLink) => parentLink.nodeId === id)
  );
                ...
                <div className='relative' ref={dropdownRef}>
                  ...
                  {parentLinks.length > 0 && (
                    <div className='mt-2 grid grid-cols-2 gap-2'>
                      {parentLinks.map((parentLink) => {
                        ...
                        const engineColor = parentNode
                          ? (ENGINE_COLORS[parentNode.engine] ?? '#6b7280')
                          : '#6b7280';
                  ...
                  {isParentDropdownOpen && availableParents.length > 0 && (
                    <div className='absolute top-full right-0 left-0 z-10 mt-1 max-h-48 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-lg dark:border-[#2d3540] dark:bg-[#1e252e]'>
                      {availableParents.map(([id, catalogNode]) => (
                        <button
                          key={id}
                          type='button'
                          onClick={() => {
                            setValue(
                              'parentLinks',
                              [...parentLinks, { nodeId: id, edgeType: 'trigger_input' }],
                              { shouldValidate: true }
                            );
                            setIsParentDropdownOpen(false);
                          }}
                        ...
                        >
                          <div
                            ...
                            style={{ background: ENGINE_COLORS[catalogNode.engine] ?? '#6b7280' }}
                          />
                          {catalogNode.label}
                        </button>
                      ))}
