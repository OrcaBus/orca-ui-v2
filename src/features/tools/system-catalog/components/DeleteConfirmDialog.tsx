...
export function DeleteConfirmDialog({ nodeLabel, onConfirm, onCancel }: DeleteConfirmDialogProps) {
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center'>
      ...
      <div className='relative w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-[#2d3540] dark:bg-[#111418]'>
        <h3 className='text-base font-bold text-slate-900 dark:text-white'>Remove Node</h3>
        <p className='mt-2 text-sm text-slate-500 dark:text-[#9dabb9]'>
          Are you sure you want to remove{' '}
          <strong className='text-slate-700 dark:text-white'>{nodeLabel}</strong>? This will also
