...
import { useAnalysisRunDetailLibrariesQueryParams } from '../hooks/useAnalysisRunDetailLibrariesQueryParams';
...
export function AnalysisRunDetailLibrariesTable() {
  const navigate = useNavigate();
  const { analysisRunDetail, isLoadingAnalysisRunDetail } = useAnalysisRunDetailContext();

  const libraryIds = analysisRunDetail?.libraries.map((lib) => lib.libraryId) || [];
  const {
    search,
    ...
    setOrderBy,
    getOrderDirection,
  } = useAnalysisRunDetailLibrariesQueryParams(libraryIds);
  ...
  if (isErrorAnalysisRunLibraries) {
    return (
      <ApiErrorState
  ...
  return (
    <>
      ...
      <DataTable
        data={analysisRunLibraries?.results || []}
        columns={columns}
        isLoading={isLoading}
        selectable
        ...
        paginationProps={{
          page: analysisRunLibraries?.pagination.page || 1,
          pageSize: analysisRunLibraries?.pagination.rowsPerPage,
          onPageChange: (page) => setPage(page),
          onPageSizeChange: (pageSize) => setRowsPerPage(pageSize),
          totalItems: analysisRunLibraries?.pagination.count || 0,
        }}
      />
    </>
  );
