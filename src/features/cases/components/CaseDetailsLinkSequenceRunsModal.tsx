...
export interface CaseDetailsLinkSequenceRunsModalProps {
  isOpen: boolean;
...
export function CaseDetailsLinkSequenceRunsModal({
  isOpen,
  alreadyLinkedIds,
  onClose,
  onSuccess,
}: CaseDetailsLinkSequenceRunsModalProps) {
  ...
  const { data: searchData, isLoading: isSearching } = useSequenceRunListModel({
    params: {
      query: {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await Promise.all(
        selectedIds.map((sequenceRunOrcabusId) =>
          linkMutation.mutateAsync({
            params: { path: { orcabusId: caseOrcabusId } },
            body: { externalEntity: sequenceRunOrcabusId },
          })
        )
      );
      toast.success(
        `${selectedIds.length} sequence ${selectedIds.length === 1 ? 'run' : 'runs'} linked`
      );
      onSuccess();
    } catch {
      toast.error('Failed to link sequence runs');
    } finally {
  ...
  return (
    <DialogFrame
      isOpen={isOpen}
      onClose={handleClose}
      title='Link Sequence Runs'
      description='Search for sequence runs by sequence run ID or instrument run ID to link to this case.'
      ...
      footer={
        <div className='flex w-full items-center justify-between'>
          <p className='text-sm text-neutral-600 dark:text-[#9dabb9]'>
            {selectedIds.length} sequence {selectedIds.length === 1 ? 'run' : 'runs'} selected
          </p>
      ...
      <div className='shrink-0 border-b border-neutral-200 px-6 py-4 dark:border-[#2d3540]'>
        <div className='relative'>
          ...
          <Input
            ...
            placeholder='Search by sequence run ID, instrument run ID...'
      ...
      <div className='min-h-0 flex-1 overflow-y-auto p-6'>
        {!searchQuery ? (
          <p className='py-6 text-center text-sm text-neutral-500 dark:text-[#9dabb9]'>
            Start typing to search for sequence runs.
          </p>
        ) : isSearching ? (
          <p className='py-6 text-center text-sm text-neutral-500 dark:text-[#9dabb9]'>
            Searching...
          ...
          <p className='py-6 text-center text-sm text-neutral-500 dark:text-[#9dabb9]'>
            No sequence runs found matching your search.
          </p>
        ) : (
